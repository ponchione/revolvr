...
- TUI-032's completion handoff published only
  ...
  unstarted; every later task remains an unpublished draft.
- Completed TUI-040 by replacing the growing managed progress-log panel with
  one typed, bounded live cell. Single pass, bounded loop, autonomous task, and
  queue modes expose task, progress/limit, elapsed time when space permits,
  admitted safety, latest meaningful detail, and the cancellation action.
- `Current:` is capped at two physical rows; repeated progress replaces it
  without appending transcript identity. Cancellation holds the exact accepted
  three-line state until matching settlement.
- Completed, failed, cancelled, blocked, safety-stop, and needs-input results
  retain the accepted committed wording. Existing append/ack settlement and
  domain callbacks are unchanged.
...
- TUI-050's completion handoff published only
  ...
  unstarted; every later task remains an unpublished draft.
- Completed TUI-051 by routing `2` and `/tasks` to the shared overlay while
  retaining the Tasks page renderer as the single content projection and D4
  rollback path.
- Task selection, notices, and Add Task confirmation remain overlay-local.
  Refresh preserves canonical task identity with the existing first-task
  fallback; retry, Add, and Workflow handoff reuse their existing callbacks,
  guards, and success/failure behavior.
- Escape restores the exact source/composer state. Focused parity covers both
  entries, 80-/40-column bounds, callback ordering, failures, and selection;
  application/domain/store authority and dependencies are unchanged.
- TUI-051's completion handoff published only
  `.agent/tasks/tui-052-move-runs-overlay.md`. TUI-052 is pending and unstarted;
  every later task remains an unpublished draft.
- Completed TUI-052 by routing `3`, `/runs`, `4`, and `/detail` to the shared
  overlay while retaining the Runs and Run Detail page renderers as the single
  content projections and D4 rollback paths.
- Runs selection/notices remain overlay-local and refresh preserves canonical
  run identity with the existing first-run fallback. `Enter`/`o` opens the
  selected detail child; back restores the same selection and list offset.
- Run Detail keeps its existing scroll, reload, timeline, raw events, artifacts,
  warnings, and receipt-validation callback. Late open/validation results
  cannot replace a newer overlay owner.
- A second Escape restores the exact source/composer state without changing
  transcript cells. Focused parity covers all entries, callbacks, 80-/40-column
  bounds, and complete evidence; application/domain authority is unchanged.
...
- TUI-053's completion handoff published only
  ...
  unstarted; every later task remains an unpublished draft.
- Completed TUI-054 by routing `6`, `/workflow`, and the Tasks handoff to the
  shared overlay while retaining the Workflow page renderer as the D4 rollback
  path.
- Selector identity, notices, scrolling, refresh, and asynchronous load results
  use overlay-local state and ownership. Late results cannot replace a newer
  overlay, and Escape restores the exact source/composer state.
- Autonomous task/queue controls, active cancellation and settlement, and the
  existing needs-input answer state machine retain their callbacks and guards.
  Live state remains visible without duplicate committed identities.
- Focused parity covers both entries, page rendering, workflow lifecycle and
  stop evidence, 80-/40-column bounds, refresh identity, typed answers, live
  settlement, exact return, and stale ownership.
...
- TUI-055's completion handoff published only
  ...
  unstarted; every later task remains an unpublished draft.
- Completed TUI-056 by routing `e` and `/evidence` to the shared overlay while
  retaining the Evidence page renderer as the D4 rollback path.
- Canonical run evidence now keeps status, diagnostics, warnings, artifact
  paths, receipt validation, and raw event identity together; autonomous
  evidence keeps task, criterion, finding, verification, source, hash, run,
  occurrence, and diagnostic identities from the existing app projection.
- Missing, invalid, warning, and satisfied evidence remain distinct text;
  stable source selection, refresh, scrolling, 80-/40-column wrapping, exact
  source/composer return, and stale ownership pass.
- Receipt validation remains callback-backed, targets only the loaded canonical
  run, and opening or navigating the overlay grants no authority.
- TUI-056's completion handoff published only
  `.agent/tasks/tui-057-move-approval-overlay.md`. TUI-057 is pending and
  unstarted; every later task remains an unpublished draft.
...
It must report TUI-057 as the only pending and next task, without selecting
...
The next fresh pass implements the already-pending TUI-057 task. Do not
republish TUI-056 or publish TUI-058 before TUI-057 completes.
...
- `go test ./... -count=1` — PASS; 4,181 tests in 91 packages.
- TUI-040 live replacement, 80-/40-column bounds, mode/pass/limit, elapsed,
  ...
  unchanged-authority, and single-publication gates — PASS.
- `go run ./cmd/revolvr task list` — PASS; TUI-040 is complete and TUI-050 is
  the only pending task.
...
- `go test ./... -count=1` — PASS; 4,196 tests in 91 packages.
- `go run ./cmd/revolvr tui --help` — PASS.
- TUI-051 Tasks-entry, retained-renderer, local-selection/notice, stable-refresh,
  Add/retry/Workflow, exact-return, 80-/40-column, no-direct-store, and
  single-publication gates — PASS.
- `go run ./cmd/revolvr task list` — PASS; TUI-051 is complete and TUI-052 is
  the only pending canonical task.
- `go run ./cmd/revolvr status` — PASS; TUI-052 is selected next.
- `.agent/STATE.md` line limit and `git diff --check` — PASS.
- `gofmt -w internal/tui/model.go internal/tui/model_test.go` — PASS.
- `go test ./internal/tui -run 'Test(Runs|RunDetail).*Overlay' -count=1` —
  PASS; 11 tests.
- `go test ./internal/tui -count=1` — PASS; 203 tests.
- `go test ./... -count=1` — PASS; 4,207 tests in 91 packages.
- `go run ./cmd/revolvr tui --help` — PASS.
...
- `go run ./cmd/revolvr task list` — PASS; TUI-052 is complete and TUI-053 is
  the only pending canonical task.
...
- `go test ./internal/tui -run 'TestWorkflow.*Overlay' -count=1` — PASS; 8
  tests.
- `go test ./internal/tui -count=1` — PASS; 219 tests.
- `go test ./... -count=1` — PASS; 4,223 tests in 91 packages.
- TUI-054 Workflow-entry, retained-renderer, selector/notice ownership,
  refresh, typed-answer, active-settlement, exact-return, 80-/40-column, and
  stale-owner gates — PASS.
- `go run ./cmd/revolvr task list` — PASS; TUI-054 is complete and TUI-055 is
  the only pending canonical task.
...
- TUI-056 Evidence-entry, retained-renderer, source-selection, provenance,
  ...
  compaction, exact-return, 80-/40-column, and stale-owner gates — PASS.
- `go run ./cmd/revolvr task list` — PASS; TUI-056 is complete and TUI-057 is
  the only pending canonical task.
- `go run ./cmd/revolvr status` — PASS; TUI-057 is selected next.
- `.agent/STATE.md` line limit, publication/status audits, and
  `git diff --check` — PASS.

Run the exact next command above for the pending TUI-057 implementation pass.

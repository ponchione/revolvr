...
`5c1c46572184e8106ba2c7e63ee6ef89dfb689d8`.

Public push-triggered CI run `30653427308`, number `150`, attempt `1`,
completed successfully for exact RC.20 source commit
`7826c0fe97bd40508553702011475cea8b35e80f`:
...
`8583930512aefa07fb281cb5daaea3af49061fda`, and its candidate-workflow bytes
still hash to
...
topology, artifact-pair, build-ID, embedded-metadata, version, vulnerability,
source/workflow identity, and RC.15-through-RC.19 preservation checks all
passed. The exact RC.20 authority remains:
...
confirmed the sole exact-source run, all ten job identities, and exactly one
...
RC.15-through-RC.19 preservation checks. Every assertion and
...
Level-1 real-Codex suite against the exact RC.20 authority above. It must
preserve RC.15 through RC.20 and all failed historical evidence, and must not
construct another candidate, create a top-level wrapper, tag, release,
external-use decision, queue, or daemon.
...
starting a model call. Clean local, fetched, raw-Git, and public `main` agree at
published candidate-record commit
`cbd54e2dac36a0ca7ba1254be036237118a7e27f`.

Public push-triggered CI run `30641614557`, number `144`, attempt `1`, completed
successfully for exact RC.18 source commit
`0bed41ef930e7db3d0486bc9a82de2b5720fe49f`:
...
successfully with the exact source SHA and one successful
`Report exact source commit` step. The source's public and local tree is
`cfb4852f07ba4e7759d94cc2890fbaa2c47bec0f`, and its candidate-workflow bytes
still hash to
`2a7ff48266b9bf3601b7f05ffef58fd36db0033a2afe2537c46043a3e69472e9`.

Post-CI workflow `--verify`, suite `--static`, direct strict complete-manifest,
topology, source/workflow, embedded-metadata, empty-build-ID, and RC.15/RC.16/
RC.17 preservation checks all passed. The exact RC.18 authority remains:

```text
/home/gernsback/source/revolvr/.revolvr/release-candidates/level1-v0.1.0-rc.18-0bed41ef930e/candidate-authority.tsv
06d8e10e6de5e0ce0774afebc0d49dc543af6334ddba0a175517329729e024ef
```
...
suite-static, strict manifest, reproducibility, embedded-metadata, empty-build-
ID, topology, source/workflow identity, and RC.15/RC.16/RC.17 preservation
checks. All passed, including `git diff --check`; no suite or model operation
occurred. The operator explicitly authorized raw-Git commit and push of this
exact three-file remote-CI record.
...
Level-1 real-Codex suite against the exact RC.18 candidate authority above. It
must preserve RC.15, RC.16, RC.17, RC.18, and all failed historical evidence;
validate every produced manifest and required scenario; and stop after this
one remaining `EXT-20` stage. It must not construct another candidate, create
a top-level wrapper, tag, release, external-use decision, queue, or daemon.

## Previous Resume Point

Exactly one new source-qualified candidate was constructed and independently
verified from clean published audit-citation repair commit
`0bed41ef930e7db3d0486bc9a82de2b5720fe49f`, tree
`cfb4852f07ba4e7759d94cc2890fbaa2c47bec0f`, without preparing a quantitative
suite or starting a model call:
...
ordinary and verbose vulnerability scans, supported-platform builds, embedded
metadata, empty build IDs, strict complete-manifest verification, and both
independent build comparisons passed. The 48-file, 137,820,906-byte bundle
contains no symlink, hard link, or residual work root. The dogfood suite's
read-only static gate passed. RC.15, RC.16, RC.17, and their failed quantitative
...
suite `--static` gates; checked strict complete-manifest verification, exact
48-file/137,820,906-byte topology, all three build pairs, embedded source
metadata, empty build IDs, retained test and vulnerability evidence, exact
local/origin/raw-Git/public source and workflow identity, and RC.15/RC.16/RC.17
preservation; and passed `git diff --check`. No fixture, suite, or model
...
That pass may obtain and record only the exact required remote CI evidence for
RC.18 and source commit `0bed41ef930e7db3d0486bc9a82de2b5720fe49f`. It
must not start live Codex, prepare a new quantitative suite, construct another
candidate, modify historical evidence, or create a tag, release, or external-
use decision. A separate later pass may start a fresh quantitative suite only
after the RC.18 remote gate passes.

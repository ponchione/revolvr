...
- TUI run-control actions remain out of scope until a future task explicitly adds them: refresh calls `internal/app.Status`, opening a selected run calls `internal/app.ShowRun`, task creation calls `internal/app.AddTask`, and no TUI action invokes Codex or `runonce`.

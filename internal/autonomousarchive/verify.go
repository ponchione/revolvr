...
func Verify(ctx context.Context, cfg VerifyConfig, selector string) (VerificationReport, error) {
	boundary, git, err := normalizeConfig(Config{RepositoryRoot: cfg.RepositoryRoot, Ledger: cfg.Ledger, GitExecutable: cfg.GitExecutable, GitTimeout: cfg.GitTimeout, CommandRunner: cfg.CommandRunner})
	if err != nil {
		return VerificationReport{}, err
	}
	storage := newArchiveStorage(boundary, nil)
	return verifyWithStorage(ctx, cfg, git, storage, selector)
}

func verifyWithStorage(ctx context.Context, cfg VerifyConfig, git gitConfig, storage *archiveStorage, selector string) (VerificationReport, error) {
	root := storage.root()
	...
	entry, err := storage.show(selector)
	if err != nil {
	...
	taskBytes, taskErr := storage.readArtifact(m.ArchivedTask)
	...
	state, stateBytes, stateErr := storage.readState(m.State.Path, m.TaskID)
	...
	if m.Disposition == DispositionCompleted {
		verifyCompleted(ctx, storage, cfg.Ledger, m, state, add)
	} else {
	...
	journal, found, journalErr := storage.loadJournal(archiveJournalPath(m.TaskID, m.OperationID))
	...
	if journalPassed {
		historyErr := verifyHistory(storage, journal)
		...
		if m.CompletionCapsule != nil {
			capsule, err := storage.readArtifact(*m.CompletionCapsule)
			if err == nil {
	...
	lineageErr := verifyActiveRelationship(storage, m, journal.CommitSHA)
	...
	secretErr := verifyForbiddenValues(storage, m, entry.ManifestBytes, cfg.ForbiddenValues)
...
func verifyCompleted(ctx context.Context, storage *archiveStorage, store Ledger, m Manifest, state autonomous.ExecutionState, add func(string, bool, string)) {
	...
	frozen, _, frozenErr := storage.readFrozen(*m.FrozenEvidence)
	...
	completionBytes, completionErr := storage.readArtifact(*m.CompletionManifest)
	...
	archivedCapsule, archiveCapsuleErr := storage.readArtifact(*m.CompletionCapsule)
	...
	activeCapsuleBytes, activeCapsuleErr := storage.readArtifact(activeCapsule)
...
func verifyHistory(storage *archiveStorage, journal Journal) error {
	for sequence := int64(1); sequence <= 5; sequence++ {
		...
		raw, found, err := storage.readRegular(rel, false)
		if err != nil {
			return err
		}
		if !found {
...
func verifyActiveRelationship(storage *archiveStorage, m Manifest, commitSHA string) error {
	root := storage.root()
...
func verifyForbiddenValues(storage *archiveStorage, m Manifest, manifestBytes []byte, forbidden []string) error {
	...
	for _, identity := range []Artifact{m.ArchivedTask} {
		raw, err := storage.readArtifact(identity)
		if err != nil {
			return err
		}
	...
	if m.CompletionCapsule != nil {
		raw, err := storage.readArtifact(*m.CompletionCapsule)
		if err != nil {
			return err
		}
		files = append(files, raw)

...
func BuildPrompt(in PromptInput) ([]byte, error) {
	...
	out.WriteString("Choose complete only when the dossier's current durable execution state already marks the plan completed, every plan step completed or skipped, and every acceptance criterion terminally dispositioned. Evidence that work satisfies a pending step or criterion does not itself change that durable status. When current evidence supports terminalizing pending plan steps or acceptance criteria, choose plan so a planner can propose those exact lifecycle updates before any later complete decision.\n\n")

...
func TestBuildPromptIsDeterministicAndIncludesExactInputs(t *testing.T) {
	...
	for _, phrase := range []string{
		...
		"Choose complete only when the dossier's current durable execution state already marks the plan completed",
		"Evidence that work satisfies a pending step or criterion does not itself change that durable status",
		"choose plan so a planner can propose those exact lifecycle updates before any later complete decision",

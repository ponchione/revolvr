...
func buildWorkerPrompt(in workerPromptInput) ([]byte, error) {
	...
	if in.Route.Action == autonomous.ActionPlan {
		...
		requiredOriginsRaw, err := marshalPromptJSON([]autonomous.EvidenceReference{taskOrigin, in.Reference.Artifact})
		if err != nil {
			return nil, err
		}
		out.WriteString("Return exactly one PlanningOutput JSON object conforming to the harness-supplied planner-only output schema, with no surrounding prose or Markdown. The object must contain the complete proposed current plan revision, complete current acceptance matrix, typed inputs, and exact provenance values shown above. Set provenance.raw_output_path to the Final-output artifact path, provenance.worker_run_id to the Worker run ID, and use the exact route, decision reference, dossier, profile, and admitted source identities. Both top-level `inputs` and `plan.provenance` must contain both exact evidence objects below. Copy each object field-for-field without changing `kind`, `reference`, or `detail`; the supervisor-decision artifact kind remains `file` and must not be relabeled as `plan`. Task-origin acceptance criteria must cite the first exact source object and repeat a requirement exactly present in the canonical task/specification:\n\n```json\n")
		out.Write(requiredOriginsRaw)
		out.WriteString("```\n\nEncode every plan-step lifecycle field exactly: pending and in_progress use `evidence: []` and `rationale: null`; completed requires nonempty terminal evidence and `rationale: null`; skipped requires a nonempty skip rationale and may include evidence. Put facts that ground future work in plan.provenance or top-level inputs, never in nonterminal step evidence. Every step in a new initial plan must be pending. In a revision, copy every existing step ID and description exactly; pending or in_progress steps may advance only when supported by the required lifecycle fields, while every terminal step must remain unchanged. Pending acceptance criteria likewise use `evidence: []` and `rationale: null`; satisfied criteria require disposition evidence; waived and not_applicable criteria require a nonempty rationale. A supervisor-refinement criterion must exactly match one current decision success_criteria string and cite the exact supervisor decision artifact. Copy every existing criterion ID, requirement, and source exactly, including an earlier supervisor-decision source; only a pending criterion may receive a supported terminal status, evidence, and rationale, while every terminal criterion must remain unchanged. Do not write a receipt; the structured planner output is the authoritative planner artifact and Revolvr will synthesize any advisory run receipt separately.\n")
	} else if in.Route.Action == autonomous.ActionAudit {

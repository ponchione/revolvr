...
func buildWorkerPrompt(in workerPromptInput) ([]byte, error) {
	...
	if in.Route.Action == autonomous.ActionPlan {
		...
		out.WriteString("```\n\nEncode every plan-step lifecycle field exactly: pending and in_progress use `evidence: []` and `rationale: null`; completed requires nonempty terminal evidence and `rationale: null`; skipped requires a nonempty skip rationale and may include evidence. Put facts that ground future work in plan.provenance or top-level inputs, never in nonterminal step evidence. Every step in a new initial plan must be pending. Pending acceptance criteria likewise use `evidence: []` and `rationale: null`; satisfied criteria require disposition evidence; waived and not_applicable criteria require a nonempty rationale. A supervisor-refinement criterion must exactly match one current decision success_criteria string and cite the exact supervisor decision artifact. Preserve every existing criterion and every terminal plan step unchanged. Do not write a receipt; the structured planner output is the authoritative planner artifact and Revolvr will synthesize any advisory run receipt separately.\n")
	} else if in.Route.Action == autonomous.ActionAudit {

...
- Codex's visible terminal UI is the deliberate visual baseline. Revolvr will
  first reproduce its layout, hierarchy, spacing, composer, status treatment,
  and interaction presentation before adding product-specific differences.
  This permits visual cloning from observation; it does not permit copying,
  vendoring, porting, embedding, or depending on Codex source.
...
- The completed transcript-first TUI overhaul is rejected as the visual
  baseline. No replacement implementation task is selected by this amendment.

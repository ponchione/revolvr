...
[[ -z "$(git status --porcelain=v1 --untracked-files=all)" ]] || {
	printf 'RC.10 construction requires a clean controller repository\n' >&2
	exit 1
}
...
[[ "$HEAD_COMMIT" == "$FETCHED_MAIN" && "$HEAD_COMMIT" == "$PUBLIC_MAIN" ]] || {
	printf 'RC.10 construction requires exact local, fetched, and public main\n' >&2
	exit 1
}
...
git diff --quiet "$SOURCE_COMMIT"..HEAD -- .agent/profiles cmd internal go.mod go.sum || {
	printf 'Product source changed after the pinned RC.10 source commit\n' >&2
	exit 1
}
...
do
	[[ ! -e "$path" && ! -L "$path" ]] || {
		printf 'Failed RC.9 final-path boundary changed: %s\n' "$path" >&2
		exit 1
	}
done

[[ ! -e agent-ext20-rc10-local-review.sh && ! -L agent-ext20-rc10-local-review.sh ]] || {
	printf 'RC.10 local-review launcher already exists\n' >&2
	exit 1
}
[[ ! -e .github/workflows/level1-rc10-candidate-attestation.yml && ! -L .github/workflows/level1-rc10-candidate-attestation.yml ]] || {
	printf 'RC.10 workflow collision\n' >&2
	exit 1
}
if git show-ref --verify --quiet refs/heads/level1-v0.1.0-rc.10 ||
	git show-ref --verify --quiet refs/heads/level1-v0.1.0-rc.10-attestation; then
	printf 'Local RC.10 ref collision\n' >&2
	exit 1
fi
REMOTE_RC10_REFS="$(git ls-remote --heads origin refs/heads/level1-v0.1.0-rc.10 refs/heads/level1-v0.1.0-rc.10-attestation)"
[[ -z "$REMOTE_RC10_REFS" ]] || {
	printf 'Remote RC.10 ref collision\n' >&2
	exit 1
}
[[ -z "$(git tag --list '*rc.10*')" ]] || {
	printf 'Local RC.10 tag collision\n' >&2
	exit 1
}
REMOTE_RC10_TAGS="$(git ls-remote --tags origin '*rc.10*')"
[[ -z "$REMOTE_RC10_TAGS" ]] || {
	printf 'Remote RC.10 tag collision\n' >&2
	exit 1
}
if find .revolvr -maxdepth 5 \( -iname '*rc10*' -o -iname '*rc.10*' \) -print -quit | grep -q .; then
	printf 'Local RC.10 runtime, diagnostic, or bundle collision\n' >&2
	exit 1
fi
if find /tmp -maxdepth 1 \( -iname 'revolvr-ext20-rc10-*' -o -iname '*level1-v0.1.0-rc.10*' \) -print -quit | grep -q .; then
	printf 'Temporary RC.10 construction-root collision\n' >&2
	exit 1
fi
...
Additional operator direction for the EXT-20 RC.10 local candidate pass:
...
- Do exactly one bounded task: construct and locally verify a fresh collision-free Level-1 candidate named level1-v0.1.0-rc.10 from exact published planner-contract remediation source commit a24804bcf2a32ee5434d3686eabad5b72d9f39ba and tree 2c8ee9f6b4283410547a9f99972e25eac06c9e33.
...
- Treat RC.1 through RC.9 and every historical candidate, ref, workflow, artifact, bundle, builder, partial or staged tree, suite, operation, launch record, diagnostic, and evidence root as immutable rejected or failed history. Never retry, resume, repair, reconcile, relabel, delete, mutate, derive candidate material from, or reuse any of it. In particular, do not execute or copy the failed RC.9 builder, staged bundles, binaries, clones, caches, or results; rebuild RC.10 independently from the pinned source.
- Preserve the RC.9 builder, diagnostic, 6-file preflight root, 34,435-file build root, 63-file stage root, 13-file staged candidate, 50-file staged verification bundle, both inventory hashes, and both final-path absences at the exact identities admitted by this launcher. Reverify them read-only before and after work.
- Preserve RC.8's builder, diagnostic, 15,882-file partial root, 20-file partial verification subtree, and final-path absences at their recorded hashes. Preserve the RC.6 and RC.7 suites, launch records, terminal evidence at their six recorded content-stream hashes, and reverify both terminal checksum manifest pairs before and after.
- Before construction, fail on any local or remote RC.10 candidate ref, attestation ref, tag, workflow, remote artifact, bundle, verification bundle, build/stage/preflight root, suite root, launch record, review launcher, or diagnostic collision. Do not overwrite partial output; after an RC.10 identity path is created, any failure exhausts it and all output must be retained unchanged.
- Preserve the clean Go boundary: every selected Go invocation explicitly uses env -u GOROOT -u GOTOOLDIR -u GOFLAGS GOENV=off GOTOOLCHAIN=local. Before creating an RC.10 build root, record and require exact executable path, SHA-256, version, GOROOT, and GOTOOLDIR for each selected toolchain. Use independent task/clone/toolchain GOCACHE and GOMODCACHE roots.
- Independent controller review reproduced that this execution environment denies moving a sealed directory from a staging parent to a sibling final path, despite compatible ownership, mode, ACL, mount, and filesystem state. It also proved that mkdir plus cp -a source/. destination/ works and preserves read-only modes. Do not use mv, rename, hard links, or symlinks to publish RC.10 bundle directories.
- Before expensive work, exercise the planned mkdir plus cp -a directory-publication method with fresh neutral empty probe paths under .revolvr/release-candidates; verify copied structure and modes, then make copied parents writable before removing only those probe paths. Record the probe. Never use an RC.10 final name for the probe and leave no probe path behind.
- Stage and seal RC.10 only in its unique fresh build root. After all staged manifests pass, create each absent exact final directory with mkdir, copy the corresponding sealed stage contents with cp -a STAGE/. FINAL/, and verify inventory files, inventory hashes, regular-file counts, complete content-stream hashes, modes, and stage/final equality at the exact final paths. Do not delete, move, repair, or complete either final path if any publication or post-copy check fails; retain a unique diagnostic and stop.
...
- Retain a new immutable RC.10 candidate bundle and separate verification bundle with exact source/tree/tool/build/version/target/environment identities, build instructions, artifact hashes, embedded metadata, tests, vulnerability result, complete sorted regular-file inventories, and inventory hashes. Verify every bundle from its manifest after construction.
- RC.10 local construction grants no remote-CI, attestation, dogfood, live-model, suite-preparation, tag, release, external-use, recovery, or queue authority. It must not create or reuse a suite.
- Keep EXT-20 unchecked. Update .agent/HANDOFF.md, .agent/STATE.md, and .agent/DECISIONS.md with exact files, hashes, final bundle paths and verification, RC.6/RC.7/RC.8/RC.9 preservation evidence, and the next independent local-review gate. Create at most one inert next-pass local-review launcher; do not execute it. Stop after this one task."

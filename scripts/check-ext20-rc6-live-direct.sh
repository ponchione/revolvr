...
readonly SCRIPT_NAME="check-ext20-rc6-live-direct"
SOURCE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
readonly SOURCE_ROOT
readonly LAUNCHER="$SOURCE_ROOT/agent-ext20-rc6-live-direct.sh"
readonly CHECK_RUN_ROOT="$SOURCE_ROOT/.revolvr/ext20-rc6.LOLauh/suite"
readonly CHECK_LAUNCH_RECORD_ROOT="$SOURCE_ROOT/.revolvr/ext20-rc6-launch-records"
TEST_ROOT=""
...
main() {
	local before_diagnostics after_diagnostics before_content after_content check_status collision_status
	TEST_ROOT="$(mktemp -d)"
	trap cleanup EXIT
	...
	# Loading definitions cannot invoke main. Exercise only the collision
	# boundary against an isolated pre-existing record.
	# shellcheck source=../agent-ext20-rc6-live-direct.sh
	source "$LAUNCHER"
	...
	printf 'RC.6 direct live gate checks passed; no live path or launch record was exercised.\n'
}

...
run_revolvr task-add task add --summary "Fake Codex verification failure smoke" "Run once verification failure through strict fake Codex"
assert_contains "$TMP_ROOT/task-add.out" "Added task "
assert_contains "$TMP_ROOT/task-add.out" "Run once verification failure through strict fake Codex"
...
assert_contains "$TMP_ROOT/status.out" "Blocked tasks: 1"
...
if [[ "$(git -C "$WORK_DIR" rev-list --count HEAD)" != "1" ]]; then
  echo "Expected failed run to leave the initial commit as HEAD" >&2
